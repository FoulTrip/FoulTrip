...
function ArticleContent({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <>
            <main className="pt-32 min-h-dvh sm:px-[5%] flex sm:flex-row flex-col gap-1 sm:h-[calc(100vh-128px)] overflow-hidden">
                                ...
                                "
                                    >
                                        <div className="flex items-center gap-1 min-w-0 sm:w-full">
                                            ...
                                            <span className="text-sm text-gray-500 font-light truncate sm:max-w-[calc(100%-4rem)]">
                                                {proyect.name}
                                            </span>
                                        </div>
                                        <HiMiniArrowTopRightOnSquare className="ml-3 shrink-0 sm:absolute sm:right-3 text-gray-400 drop-shadow-md" />
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <article className="grow px-10 overflow-y-auto h-full">
                    <div className="max-w-prose mx-auto">
                        <p className="text-gray-500 sm:mt-0 mt-20">
                            Publicado el {article?.date} en <span className="cursor-pointer hover:text-blue-400 underline">{article?.type}</span>
                        </p>
                        {article?.mdxSource && (
                            <div className="mt-6">
